import { GET_ALL_LINKED_ITEM_LIST } from '../../sql/sqlScript';
...
export const getAllLinkedItems = async (
  ...
  try {
    // Fetch lead count
    const res = await client.query(GET_ALL_LINKED_ITEM_LIST);
    const result = {
      list: res?.rows || [],
    };
    return result;
  } catch (error: any) {
    logger.error('Failed to fetch linked item list', { error });
    throw new Error(`Failed to fetch linked item list: ${error.message}`);
  } finally {
    try {
      await client.end();
    } catch (endError: any) {
