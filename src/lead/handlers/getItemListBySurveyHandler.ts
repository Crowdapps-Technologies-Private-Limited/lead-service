...
import { getSurveyedItems } from '../services';
import { RouteHandler } from '../../types/interfaces';
import logger from '../../utils/logger';

export const getItemListBySurveyHandler: RouteHandler = async (
    event: APIGatewayProxyEventBase<APIGatewayEventDefaultAuthorizerContext>,
): Promise<APIGatewayProxyResult> => {
    logger.info('getItemListBySurveyHandler event ', { event });  
    try {
        ...
        if (!surveyId || !leadId) {
            return ResponseHandler.badRequestResponse({ message: `Survey ID and Lead ID are required in path parameters` });
        }
        const tenant = (event.requestContext as any).tenant;

        // Fetch user list
        const result = await getSurveyedItems(leadId, surveyId, tenant);
