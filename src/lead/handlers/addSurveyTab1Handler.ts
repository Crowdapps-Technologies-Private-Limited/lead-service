...
import { validateAddSurveyTab1Payload } from '../validator';
import { addSurveyTab1 } from '../services';
...
export const addSurveyTab1Handler: RouteHandler = async (
    event: APIGatewayProxyEventBase<APIGatewayEventDefaultAuthorizerContext>,
): Promise<APIGatewayProxyResult> => {
    logger.info('addSurveyTab1Handler event ', { event });  
    try {
        ...
        // Validate payload
        try {
            await validateAddSurveyTab1Payload(payload);
        } catch (error: any) {
        ...
        const result = await addSurveyTab1(leadId, payload, tenant);
